...
freq=1e-12; %logspace(10,14, 20)';
for freq_for=1:1%length(freq)
    ...
    % initial value
    solution_vector_AC=zeros(length(solution_vector_Vd),length(solution_vector_Vd));
    ...
    for ii=1:length(Element)
        ...
        %% res vector
        res_AC=diag(ones(length(solution_vector_possion),1));
...
% node=193;
% 
% Visual_solution_vector_AC=solution_vector_AC(:,node);
% 
% figure(1) % 점으로 3D
% plot3(Vertex(:,1),Vertex(:,2),Visual_solution_vector_AC(:,1),'*')
% 
% figure(2); % mesh 모양
...
% 
% figure(3); % potential
% patch('Faces',Element,'Vertices',Vertex(:,[1 2]), 'FaceVertexCData',Visual_solution_vector_AC(:,1), 'EdgeColor','black','FaceColor','interp');
