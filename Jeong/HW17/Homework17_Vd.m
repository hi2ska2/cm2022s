clear; clc; close all;
load('./data/Homework17_dd.mat')
mkdir('./data/Homework17_Vd')
...
for bias_Vd=1:6 %%% Vd는 1V까지 올림.
    ...
    for Newton_Vd=1:Iteration_Vd
        ...
        update_Vd_elec_for_break=update_Vd(find(Table_Jaco(:,1)==1, 1 , 'first')+1:3:find(Table_Jaco(:,1)==1, 1 , 'last')+1,Newton_Vd);
        update_Vd_hole_for_break=update_Vd(find(Table_Jaco(:,1)==1, 1 , 'first')+2:3:find(Table_Jaco(:,1)==1, 1 , 'last')+2,Newton_Vd);
        fprintf("Max Updatevector=%d\n\n" , max(abs(update_Vd_phi_for_break)))
        if max(abs(update_Vd_phi_for_break))<1e-15 && max(abs(update_Vd_elec_for_break))<1e10 && max(abs(update_Vd_hole_for_break))<1e-2
            break;
        end
...
% 
%     % Save
%     FILENAME = sprintf('./data/Homework17_Vd/Homework17_Vd_%.2fV.mat' , Vd);
%     save(FILENAME);
% 
%     FILENAME = sprintf('./data/Homework17_Vd/Homework17_Vd_%.2fV_current.mat',Vd);
%     save(FILENAME, 'I_Vd');
