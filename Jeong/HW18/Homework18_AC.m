...
for freq_for=1:length(freq)
    ...
    clearvars -except freq freq_for I_Sg Admittance Admittance_real Admittance_imag
    load('Ramping_0.5V_data.mat')
    ...
    % initial value
    solution_vector_Sg=zeros(length(solution_vector_Vd),1);

    % 여기서 나오는 Jaco, res는 사실 AC에서는 A matrix, b vector이다.(oneshot이므로)
    Jaco_Sg=zeros(size(solution_vector_Sg,1),size(solution_vector_Sg,1)); res_Sg=zeros(size(solution_vector_Sg,1),1);
    % Jaco/res 작성 %
    for ii=1:length(Element)
        ...
        %% res vector
        res_tmp_Sg=zeros(9,1); % oneshot에 푸는것임. 사실 res가 아니라 b vector.
        ...
        Jaco_tmp_Sg(2,2)=Jaco_tmp_Sg(2,2)-q*Control_Volume(1,1)*1i*w;
        ...
        Jaco_tmp_Sg(3,3)=Jaco_tmp_Sg(3,3)+q*Control_Volume(1,1)*1i*w;
        ...
        Jaco_tmp_Sg(5,5)=Jaco_tmp_Sg(5,5)-q*Control_Volume(2,1)*1i*w;
        ...
        Jaco_tmp_Sg(6,6)=Jaco_tmp_Sg(6,6)+q*Control_Volume(2,1)*1i*w;
        ...
        Jaco_tmp_Sg(8,8)=Jaco_tmp_Sg(8,8)-q*Control_Volume(3,1)*1i*w;
        ...
        Jaco_tmp_Sg(9,9)=Jaco_tmp_Sg(9,9)+q*Control_Volume(3,1)*1i*w;
    ...
    for ii=1:size(anode_BC,1)
        ...
        res_Sg(BC_index+1,1)=solution_vector_Sg(BC_index+1,1)-0;
        res_Sg(BC_index+2,1)=solution_vector_Sg(BC_index+2,1)-0;
    end
    ...
    for ii=1:size(cathode_BC,1)
        ...
        Jaco_Sg(BC_index,:)=0; Jaco_Sg(BC_index+1,:)=0; Jaco_Sg(BC_index+2,:)=0;
        Jaco_Sg(BC_index,BC_index)=1; Jaco_Sg(BC_index+1,BC_index+1)=1; Jaco_Sg(BC_index+2,BC_index+2)=1;
        res_Sg(BC_index,1)=solution_vector_Sg(BC_index,1)-1i*Amp;
        res_Sg(BC_index+1,1)=solution_vector_Sg(BC_index+1,1)-0;
        res_Sg(BC_index+2,1)=solution_vector_Sg(BC_index+2,1)-0;
    end
    ...
    update_Sg(:,1)=Cmatrix_Sg*update_scaled_Sg;

    solution_vector_Sg(:,1)=update_Sg(:,1);


    %% Current %%
    I_n_Sg(1,1)=0; I_p_Sg(1,1)=0; I_d_Sg(1,1)=0;
    for ii=1:size(cathode_BC,1)
        ...
        for k=1:size(row,1)
            ...
            I_n_Sg(1,1)=I_n_Sg(1,1)+I_n_Sg_tmp*width;
            I_p_Sg(1,1)=I_p_Sg(1,1)+I_p_Sg_tmp*width;
            I_d_Sg(1,1)=I_d_Sg(1,1)+I_d_tmp*width;


        end
    end
    I_Sg(1,freq_for) = (I_n_Sg(1,1)+I_p_Sg(1,1)+I_d_Sg(1,1));

    Admittance(freq_for,1)=I_Sg(1,freq_for)/(Amp*1i);
