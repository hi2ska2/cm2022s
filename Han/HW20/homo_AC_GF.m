...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Residue matrix
Res_ac = eye(size(Jaco_ac,1),size(Jaco_ac,1));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Dirichlet boundary 
for i=1:(Ncon1+Ncon2)
    for j=1:concol
        ...
        if i<=Ncon1 % Cathode
            Res_ac(X2(3*fcon-2),:) = 0;
            Res_ac(X2(3*fcon-1),:) = 0;
            Res_ac(X2(3*fcon),:) = 0;

        elseif i>(Ncon1) &&  i<=(Ncon1+Ncon2) % Anode
            Res_ac(X2(3*fcon-2),:) = Vamp*inum;
            Res_ac(X2(3*fcon-1),:) = 0;
            Res_ac(X2(3*fcon),:) = 0;
        end
    end
...
Jaco_ac_scaled = Jaco_ac * Cmatrix;
Rvector = 1./sum(abs(Jaco_ac_scaled),2);
Rmatrix = spdiags(Rvector,0,size(X2_sorted,1),size(X2_sorted,1));

Jaco_ac_scaled = Rmatrix * Jaco_ac_scaled;
Res_ac_scaled = Rmatrix * Res_ac;

sol_ac_scaled = Jaco_ac_scaled \ (Res_ac_scaled);
sol_ac = Cmatrix * sol_ac_scaled;

for ii=1:VErow
    dphi(ii,1) = sol_ac(3*ii-2,1);
    dn(ii,1) = sol_ac(3*ii-1,1);
    dp(ii,1) = sol_ac(3*ii,1);
end
...
ansmat = zeros(3,1);
ansmat(1,1) = max(I);
ansmat(2,1) = max(abs(In));
ansmat(3,1) = max(abs(Id));
