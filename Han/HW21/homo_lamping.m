...
Nd = 2e23;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
V = importdata("homo_vertex.txt"); Vrow = size(V,1);
E = importdata("homo_element.txt"); Erow = size(E,1);
E2 = importdata("homo_element_region2.txt"); E2row = size(E2,1);
E3 = importdata("homo_element_region3.txt"); E3row = size(E3,1);
E4 = importdata("homo_element_region4.txt"); E4row = size(E4,1);
Esi = vertcat(E2, E3, E4); Esirow = size(Esi,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% edge contact information
contact = importdata("homo_contact.txt");
[conrow, concol] = size(contact);
unicon = unique(contact);
Ncon1 = 10;
Ncon2 = 10;

% Cathode contact analysis
conout = zeros(2*Ncon1,1);
aaa=1;
for i=1:Ncon1
    for j=1:concol
        conout(aaa,1) = contact(i,j);
        aaa = aaa+1;
    end
end
uniconout = unique(conout);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% jacobian, residue matrix (non-linear Possion) %%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for it=1:Niter
    ...
    for i=1:Esirow
        ...
        elseif i > (E2row+E3row) && i <= Esirow
            ...
            jac(X2(3*X1_sorted(Esi(i,3),2)-2,1),X2(3*(X1_sorted(Esi(i,1),2))-2,1)) = jac(X2(3*(X1_sorted(Esi(i,3),2))-2,1),X2(3*(X1_sorted(Esi(i,1),2))-2,1))...
                + esi*edgeEsi(i,3)/lenEsi(i,3);
            res(X2(3*X1_sorted(Esi(i,3),2)-2,1),1) =  res(X2(3*X1_sorted(Esi(i,3),2)-2,1),1)...
                + (edgeEsi(i,2)*lenEsi(i,2) + edgeEsi(i,3)*lenEsi(i,3))*0.25*q/e0*Nd...
                - esi*(edgeEsi(i,2)/lenEsi(i,2) + edgeEsi(i,3)/lenEsi(i,3))*phi(X1_sorted(Esi(i,3),2))...
                + esi*edgeEsi(i,2)/lenEsi(i,2)*phi(X1_sorted(Esi(i,2),2))...
                + esi*edgeEsi(i,3)/lenEsi(i,3)*phi(X1_sorted(Esi(i,1),2));
    ...
    for i=1:size(X2_sorted,1)
        if rem(X2_sorted(i),3) == 1
            ...
            if maxupd < max(abs(upd(i)))
                maxupd = max(abs(upd(i)));
            end
...
save homo_ramping_information.mat