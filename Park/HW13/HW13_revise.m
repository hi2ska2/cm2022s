...
vertex = textscan(fileID,'%f%f%f','Delimiter','\t');

Vertex = horzcat(vertex{1},vertex{2},vertex{3});
fclose(fileID);
...
fileID8 = fopen(['Contact_Si.txt']);
contact1 = textscan(fileID8,'%f%f','Delimiter','\t');

Contact_Si = horzcat(contact1{1},contact1{2});
fclose(fileID8);
...
iy1 = [73; 104; 135; 166; 197; 228];
iy2 = [83; 114; 145; 176; 207; 238];

ry1 = unique(Contact_Si(1:5,:)); % Source
ry2 = unique(Contact_Si(6:10,:)); % Drain
...
phi(260:268,1) = Thermal_V*log((Na)/nint);
phi(291:299,1) = Thermal_V*log((Na)/nint);

for ii = 1:6
        phi(iy1(ii,1)+1:iy2(ii,1)-1,1) = Thermal_V*log((Na)/nint);
end
...
%    Boundary Condition
    ...
    for n=1:C_col
        for i =1: C_row
            if i <= 10 % Bottom Gate
                ...
                Jaco(434+Contact(i,n),:) =0;
                Jaco(434+Contact(i,n),434+Contact(i,n)) = 1;
                res(434+Contact(i,n),1)= phi(62+Contact(i,n),1)-0.33374;
            end
        end
    end
   ...
   %     scaling
        ...
        elec(94:1:279,1) = elec(94:1:279,1) + update_vector(2+VR1:3:VR1+3*VR2,Newton);
        hole(94:1:279,1) = hole(94:1:279,1) + update_vector(3+VR1:3:VR1+3*VR2,Newton);
...
for bias = 0:10
    ...
    for Newton = 1:20
        ...
        if norm(update_DD_vector_Drain(1+VR1:3:VR1+3*VR2,1),inf) < 1e-15
            break;
        end
