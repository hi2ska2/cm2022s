Vd=1.0;   % Drain Voltage

%%%%% transient information %%%%%
time_x = 0.1e-9;

Time = 5*time_x; % Total time
R_time = 1*time_x; % rising time
F_time = 3*time_x; % falling time

deltat = 0.1*time_x;
Step = round(Time/deltat);
...
for T_step = 1:Step+1
    ...
    if R_time <= t(:,T_step) && t(:,T_step) <= R_time + 1*time_x
        Vg = 1/(1*time_x)*(t(:,T_step)-1*time_x);

    elseif R_time + 1*time_x < t(:,T_step) && t(:,T_step) <= F_time
        Vg = 1;

    elseif F_time < t(:,T_step) && t(:,T_step) <= F_time + 1*time_x
        Vg = -1/(1*time_x)*(t(:,T_step)-4*time_x);
    else
        Vg = 0;
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for Newton = 1:20
                            ...
                            elseif Table(ii,3) == eDensity
                                ...
                                for rr = 1:K_row
                                    if K(rr,1) <= R_row2
                                        ...
                                        % transient
                                        res(ii,1) = res(ii,1) - (q/4)/deltat*(Area(K(rr,1)+R_row1,jj)*L(K(rr,1)+R_row1,jj)+Area(K(rr,1)+R_row1,jj+2)*L(K(rr,1)+R_row1,jj+2))*(elec(VR1+V1,1)-elec_old(VR1+V1,1));
                                    ...
                                    elseif K(rr,1) > R_row2 && K(rr,1) <= 2*R_row2
                                        ...
                                        % transient
                                        res(ii,1) = res(ii,1) - (q/4)/deltat*(Area(K(rr,1)+R_row1,jj-1)*L(K(rr,1)+R_row1,jj-1)+Area(K(rr,1)+R_row1,jj)*L(K(rr,1)+R_row1,jj))*(elec(VR1+V2,1)-elec_old(VR1+V2,1));
                                    ...
                                    elseif K(rr,1) > 2*R_row2 && K(rr,1) <= 3*R_row2
                                        ...
                                        % transient
                                        res(ii,1) = res(ii,1) - (q/4)/deltat*(Area(K(rr,1)+R_row1,jj)*L(K(rr,1)+R_row1,jj)+Area(K(rr,1)+R_row1,jj-1)*L(K(rr,1)+R_row1,jj-1))*(elec(VR1+V3,1)-elec_old(VR1+V3,1));
                            ...
                            elseif Table(ii,3) == hDensity
                                ...
                                for rr = 1:K_row
                                    ...
                                    if K(rr,1) <= R_row2
                                        ...
                                        % transient
                                        res(ii,1) = res(ii,1) + (q/4)/deltat*(Area(K(rr,1)+R_row1,jj)*L(K(rr,1)+R_row1,jj)+Area(K(rr,1)+R_row1,jj+2)*L(K(rr,1)+R_row1,jj+2))*(hole(VR1+V1,1)-hole_old(VR1+V1,1));
                                    ...
                                    elseif K(rr,1) > R_row2 && K(rr,1) <= 2*R_row2
                                        ...
                                        % transient
                                        res(ii,1) = res(ii,1) + (q/4)/deltat*(Area(K(rr,1)+R_row1,jj-1)*L(K(rr,1)+R_row1,jj-1)+Area(K(rr,1)+R_row1,jj)*L(K(rr,1)+R_row1,jj))*(hole(VR1+V2,1)-hole_old(VR1+V2,1));
                                    ...
                                    elseif K(rr,1) > 2*R_row2 && K(rr,1) <= 3*R_row2
                                        ...
                                        % transient
                                        res(ii,1) = res(ii,1) + (q/4)/deltat*(Area(K(rr,1)+R_row1,jj)*L(K(rr,1)+R_row1,jj)+Area(K(rr,1)+R_row1,jj-1)*L(K(rr,1)+R_row1,jj-1))*(hole(VR1+V3,1)-hole_old(VR1+V3,1));
            ...
            for n=1:C_col
                for i =1: C_row
                    if i <= (C_row/2) % Bottom Gate
                        ...
                        res(VR1+3*VR2+V,1)= phi(VR1+VR2+V,1)-0.33374-Vg;
                    end
                end
            end
            ...
            phi(1+VR1+VR2:T,1) = phi(1+VR1+VR2:T,1) + update_DD_vector_Gate(1+VR1+3*VR2:T_row,Newton);
        ...
        for tt=1:w_row
            ...
            for rr = 1:K_row
                ...
                if K(rr,1) <= R_row2
                    ...
                    Jn(T_step,1) = Jn(T_step,1) + coeff_Jn*(elec(VR1+V2,1)*Ber(x21) - elec(VR1+V1,1)*Ber(x12))*(Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj));
                    Jn(T_step,1) = Jn(T_step,1) + coeff_Jn*(elec(VR1+V3,1)*Ber(x31) - elec(VR1+V1,1)*Ber(x13))*(Area(K(rr,1)+R_row1,jj+2)/L(K(rr,1)+R_row1,jj+2));
                    Jp(T_step,1) = Jp(T_step,1) + coeff_Jp*(hole(VR1+V2,1)*Ber(x12) - hole(VR1+V1,1)*Ber(x21))*(Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj));
                    Jp(T_step,1) = Jp(T_step,1) + coeff_Jp*(hole(VR1+V3,1)*Ber(x13) - hole(VR1+V1,1)*Ber(x31))*(Area(K(rr,1)+R_row1,jj+2)/L(K(rr,1)+R_row1,jj+2));
                    Jd(T_step,1) = Jd(T_step,1) - esi*e0*((phi(VR1+V3,1)-phi(VR1+V1,1))-(phi_old(VR1+V3,1)-phi_old(VR1+V1,1)))*Area(K(rr,1)+R_row1,jj+2)/L(K(rr,1)+R_row1,jj+2);
                    Jd(T_step,1) = Jd(T_step,1) - esi*e0/deltat*((phi(VR1+V2,1)-phi(VR1+V1,1))-(phi_old(VR1+V2,1)-phi_old(VR1+V1,1)))*Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj);
                ...
                elseif K(rr,1) > R_row2 && K(rr,1) <= 2*R_row2
                    ...
                    Jn(T_step,1) = Jn(T_step,1) + coeff_Jn*((Area(K(rr,1)+R_row1,jj-1)/L(K(rr,1)+R_row1,jj-1))*(elec(VR1+V1,1)*Ber(x12) - elec(VR1+V2,1)*Ber(x21)));
                    Jn(T_step,1) = Jn(T_step,1) + coeff_Jn*((Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj))*(elec(VR1+V3,1)*Ber(x32) - elec(VR1+V2,1)*Ber(x23)));
                    Jp(T_step,1) = Jp(T_step,1) + coeff_Jp*((Area(K(rr,1)+R_row1,jj-1)/L(K(rr,1)+R_row1,jj-1))*(hole(VR1+V1,1)*Ber(x21) - hole(VR1+V2,1)*Ber(x12)));
                    Jp(T_step,1) = Jp(T_step,1) + coeff_Jp*((Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj))*(hole(VR1+V3,1)*Ber(x23) - hole(VR1+V2,1)*Ber(x32)));
                    Jd(T_step,1) = Jd(T_step,1) - esi*e0/deltat*((phi(VR1+V1,1)-phi(VR1+V2,1))-(phi_old(VR1+V1,1)-phi_old(VR1+V2,1)))*Area(K(rr,1)+R_row1,jj-1)/L(K(rr,1)+R_row1,jj-1);
                    Jd(T_step,1) = Jd(T_step,1) - esi*e0/deltat*((phi(VR1+V3,1)-phi(VR1+V2,1))-(phi_old(VR1+V3,1)-phi_old(VR1+V2,1)))*Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj);
                ...
                elseif K(rr,1) > 2*R_row2 && K(rr,1) <= 3*R_row2
                    ...
                    Jn(T_step,1) = Jn(T_step,1) + coeff_Jn*((Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj))*(elec(VR1+V1,1)*Ber(x13) - elec(VR1+V3,1)*Ber(x31)));
                    Jn(T_step,1) = Jn(T_step,1) + coeff_Jn*((Area(K(rr,1)+R_row1,jj-1)/L(K(rr,1)+R_row1,jj-1))*(elec(VR1+V2,1)*Ber(x23) - elec(VR1+V3,1)*Ber(x32)));
                    Jp(T_step,1) = Jp(T_step,1) + coeff_Jp*((Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj))*(hole(VR1+V1,1)*Ber(x31) - hole(VR1+V3,1)*Ber(x13)));
                    Jp(T_step,1) = Jp(T_step,1) + coeff_Jp*((Area(K(rr,1)+R_row1,jj-1)/L(K(rr,1)+R_row1,jj-1))*(hole(VR1+V2,1)*Ber(x32) - hole(VR1+V3,1)*Ber(x23)));
                    Jd(T_step,1) = Jd(T_step,1) - esi*e0/deltat*((phi(VR1+V1,1)-phi(VR1+V3,1))-(phi_old(VR1+V1,1)-phi_old(VR1+V3,1)))*Area(K(rr,1)+R_row1,jj)/L(K(rr,1)+R_row1,jj);
                    Jd(T_step,1) = Jd(T_step,1) - esi*e0/deltat*((phi(VR1+V2,1)-phi(VR1+V3,1))-(phi_old(VR1+V2,1)-phi_old(VR1+V3,1)))*Area(K(rr,1)+R_row1,jj-1)/L(K(rr,1)+R_row1,jj-1);
                end
